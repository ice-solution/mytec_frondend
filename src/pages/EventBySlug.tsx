...
const EventBySlug = () => {
  ...
  const { categories, slug } = useParams<{ categories: string; slug: string }>();
  ...
  useEffect(() => {
    const fetchEventBySlug = async () => {
      if (!categories || !slug) {
        console.error('Missing categories or slug');
        return;
      }
      
      try {
        // 調用 API 獲取活動詳情，使用 categories 和 slug
        console.log('Fetching event by slug:', { categories, slug });
        const response = await api.get(`/events/${categories}/${slug}`);
        
        if (response.data) {
          const event = response.data;
          setEventId(event._id || event.id);
          
          // 手動設置 eventData，因為我們不是通過 useApi 調用的
          execute(Promise.resolve({ data: event }));
